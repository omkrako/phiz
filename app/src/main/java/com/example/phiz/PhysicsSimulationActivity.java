...
public class PhysicsSimulationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        simulationView.setOnPhysicsUpdateListener((acceleration, velocity, position) -> {
            runOnUiThread(() -> {
                accelerationTextView.setText(String.format(Locale.US,
                        "Acceleration: %.2f m/s²", acceleration));
                velocityTextView.setText(String.format(Locale.US,
                        "Velocity: %.2f m/s", velocity));
                positionTextView.setText(String.format(Locale.US,
                        "Position: %.2f m", position));
            });
        });
